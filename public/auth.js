...
function renderRoutes() {
    ...
    container.innerHTML = routesData.map(route => `
        <div class="route-card" onclick="startBooking(${route.id})">
            ...
            <div style="color:#FFB347;font-weight:bold;font-size:1.2rem;margin-top:10px">${route.price} ₽</div>
...
window.startBooking = function(routeId) {
    if (!currentUser) { showNotification('Сначала войдите!', 'error'); window.openAuthModal('login'); return; }
...
function openSeatModal() {
    ...
    modal.innerHTML = `
        <div class="qr-content" style="color:#0f172a;text-align:left;max-width:400px">
            ...
            <p style="color:#64748b;margin-bottom:15px">💰 ${selectedRoute.price} ₽ | 🕐 ${selectedRoute.time}</p>
    ...
    for (let i = 1; i <= 12; i++) {
        ...
        seat.className = 'seat';
        seat.textContent = i;
        const isBooked = bookings.some(b => b.routeId === selectedRoute.id && b.seatNumber === i && b.status === 'paid');
...
function generateFakeQR(canvasId) {
    ...
    function drawMarker(x, y) {
        ...
        ctx.fillStyle = '#ffffff'; ctx.fillRect(x+cellSize, y+cellSize, cellSize*5, cellSize*5);
        ctx.fillStyle = '#000000'; ctx.fillRect(x+cellSize*2, y+cellSize*2, cellSize*3, cellSize*3);
    }
    drawMarker(0, 0); drawMarker(size-cellSize*7, 0); drawMarker(0, size-cellSize*7);
...
function confirmPayment() {
    const newBooking = {
        id: Date.now(), userId: currentUser.id, routeId: selectedRoute.id,
        routeName: `${selectedRoute.from} → ${selectedRoute.to}`,
        seatNumber: selectedSeat, price: selectedRoute.price,
        date: new Date().toLocaleDateString(), status: 'paid',
        ticketNumber: `TKT-${Math.floor(100000 + Math.random() * 900000)}`
    };
    addBooking(newBooking);
    closeModal('paymentModal');
    showNotification('Билет успешно оплачен! ', 'success');
    setTimeout(() => window.switchTab('bookings'), 800);
}
...
function addBooking(b) { bookings.push(b); saveBookings(); renderMyBookings(); }
function getUserBookings(uid) { return bookings.filter(b => b.userId === uid); }

function renderMyBookings() {
    const container = document.getElementById('bookingsContainer');
    if (!container) return;
    if (!currentUser) { container.innerHTML = '<p style="color:#94a3b8;text-align:center;padding:2rem">Войдите, чтобы увидеть билеты</p>'; return; }
...
window.processRefund = function(bookingId) {
    ...
    if (cleanCard.length !== 16) { errorEl.textContent = 'Введите корректный номер карты (16 цифр)'; return; }
...
window.switchTab = function(tabName) {
    ...
    if (tabName === 'bookings') { btns[1]?.classList.add('active'); renderMyBookings(); }
    if (tabName === 'about') btns[2]?.classList.add('active');
};

window.logoutUser = function() {
    currentUser = null; saveUserSession(); updateAuthUI();
    showNotification('Вы вышли из аккаунта', 'info');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (nameDisplay) nameDisplay.onclick = () => currentUser ? window.switchTab('bookings') : window.openAuthModal('login');
});